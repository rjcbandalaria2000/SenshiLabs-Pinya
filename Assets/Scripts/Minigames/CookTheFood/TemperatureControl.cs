...
public class TemperatureControl : MonoBehaviour
{
    ...
    public float                speedJitterTime = 0.5f;
    ...
    private int                 randomSpeedIndex = 0;
    private Vector3             destination;
    private Coroutine           moveTrackerRoutine;
    private Coroutine           setJitterSpeedRoutine;
    private Pot                 pot;
    ...
    public void StartSetJitterSpeed()
    {
        setJitterSpeedRoutine = StartCoroutine(SetTrackerJitterSpeed());
    }
    ...
    IEnumerator MoveTracker()
    {
        while (true)
        {
            ...
            Tracker.transform.position = Vector2.MoveTowards(Tracker.transform.position,
                destination,
                Speeds[randomSpeedIndex] * Time.deltaTime);
        ...
        if(ChosenTemp != null) {
            ...
            if (pointCounter >= RequiredPointCounter)
            {
              ...
              //  sFX.ChangePlayMusic(cookingSFX[RequiredPointCounter - 1]);
                ...
                StopCoroutine(setJitterSpeedRoutine);
    ...
    IEnumerator SetTrackerJitterSpeed()
    {
        while (true)
        {
            randomSpeedIndex = Random.Range(0, Speeds.Count);
            yield return new WaitForSeconds(speedJitterTime);
        }
